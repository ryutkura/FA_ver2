...
/**
 * Self-Adaptive Fireworks Algorithm (SaFWA) の実行クラス
 */
public class SaFWARunner {
    
    public static void main(String[] args) {
        // 実験パラメータ
        int dimension = 2;
        int runs = 30;
        int maxEvaluations = 10000*dimension;
        ...
        for (int p = 0; p < problems.length; p++) {
            ...
            for (int r = 0; r < runs; r++) {
                ...
                // SaFWAのインスタンス作成
                SaFWAAlgorithm safwa = new SaFWAAlgorithm(problem);
                safwa.setMaxEvaluations(maxEvaluations);
                safwa.setParameters(//元ネタ
                    10,           // populationSize (n)
                    maxEvaluations, // maxEvaluations
                    90,           // totalSparks (m)
                    0.04,         // boundingCoeffA (a)
                    0.8,          // boundingCoeffB (b)
                    2.0,          // maxAmplitude (Â)
                    8,            // gaussianSparks
                    10            // learningPeriod (LP)
                );
                
                // アルゴリズムの実行と時間計測
                long startTime = System.currentTimeMillis();
                double[] solution = safwa.optimize();
